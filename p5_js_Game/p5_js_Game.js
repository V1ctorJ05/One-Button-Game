...
function draw() {
  ...
  // Make green stars appear more often after 100 points
  if (score >= 100) {
    greenProb = 0.02; // increase green star chance
  }
  ...
  for (let i = 0; i < numStars; i++) {
    ...
    if (isBonus[i]) {
      ...
      let tw = starSize[i] + sin(frameCount * 0.3 + i) * 6;
      fill(starColor[i]);
      circle(starX[i], starY[i], tw);
    } else {
...
function resetStar(i) {
  ...
  let r = random();
  if (r < blueProb) {
    starColor[i] = color(0, 0, 255);
    isBonus[i] = true;
    isPenalty[i] = false;
    isDanger[i] = false;
    isYellow[i] = false;
    starSpeed[i] = blueSpeed;
  } else if (r < blueProb + greenProb) {
    starColor[i] = color(0, 255, 0);
    isBonus[i] = false;
    isPenalty[i] = true;
    isDanger[i] = false;
    isYellow[i] = false;
    starSpeed[i] = greenSpeed;
  } else if (r < redProb + blueProb + greenProb) {
    starColor[i] = color(255, 0, 0);
    isBonus[i] = false;
    isPenalty[i] = false;
    isDanger[i] = true;
    isYellow[i] = false;
    starSpeed[i] = redSpeed;
  } else {
...
function mousePressed() {
  ...
  if (!gameStarted) {
    ...
    if (instr) instr.style.display = "none"; // hide HTML instructions
    return;
  }
